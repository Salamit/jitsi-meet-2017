...
(function() {
    ...
    const room = s.substr(s.lastIndexOf('/') + 1);

    const styleSheet = '/togetherjs/togetherjs.css';

    let baseUrl = 'https://togetherjs.com';
    ...
    let min = '__' + 'min__' == 'yes' ? false : 'yes' === 'yes';
    ...
    if (configOverride) {
        ...
        if (! configOverride || configOverride.expiresAt < Date.now()) {
            ...
            for (const attr in configOverride) {
    ...
    const TogetherJS = window.TogetherJS = function TogetherJS(event) {
        ...
        const ons = TogetherJS.config.get('on');
        ...
        const hubOns = TogetherJS.config.get('hub_on');
        ...
        addStyle();
        const minSetting = TogetherJS.config.get('useMinimizedCode');

        TogetherJS.config.close('useMinimizedCode');
        if (minSetting !== undefined) {
            min = Boolean(minSetting);
        }
        const requireConfig = TogetherJS._extend(TogetherJS.requireConfig);
    ...
    TogetherJS._teardown = function() {
        const requireObject = TogetherJS._requireObject || window.require;
    ...
    TogetherJS.config = function(name, maybeValue) {
        ...
        for (var attr in settings) {
            ...
            const previous = TogetherJS._configuration[attr];
            const value = settings[attr];

            TogetherJS._configuration[attr] = value;
            const trackers = TogetherJS._configTrackers[name] || [];
            let failed = false;
    ...
    TogetherJS.reinitialize = function() {
        if (TogetherJS.running && typeof TogetherJS.require === 'function') {
            TogetherJS.require([ 'session' ], session => {
    ...
    TogetherJS.config.track('enableShortcut', (enable, previous) => {
        if (enable) {
    ...
    /* if (m) {
        TogetherJS.startup._joinShareId = room;
